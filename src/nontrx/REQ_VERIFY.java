 ...
 * @author VINCENT
 */
public class REQ_VERIFY extends Konf{
    ...
    public REQ_VERIFY(String idv,String cntrv) throws Exception {
        
        try{
            //Class.forName("com.mysql.jdbc.Driver");
            con = DriverManager.getConnection(DB_URL,DB_USER,DB_PASSWORD);
            
            statv = con.prepareCall("{call VERIF1(?,?,?,?,?) }");
            statv.setString(1, idv);
            statv.setString(2, dateString);
        ...
        if(hasilx.equalsIgnoreCase("1")) {
                ...
                jo.put("com","VERIFY_NUMB");
                jo.put("counter",Integer.parseInt(cntrv));
                
                Smser sms = new Smser();
                String abc = idv.replaceFirst("0","+62");
                sms.sendPost(abc,id);
                ...
                jo.put("datetime",dateString);
                jo.put("result","Nomor Anda Telah Terdaftar");
                jo.put("counter",Integer.parseInt(cntrv));
                jo.put("com","VERIFY_NUMB");
