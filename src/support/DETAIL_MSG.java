 ...
 * @author VINCENT
 */
public class DETAIL_MSG extends Konf{
    public String hasilx,stv,ans,ctrku;
    ...
    public DETAIL_MSG(String getidku,String getctr) {
        
        try{
            //Class.forName("com.mysql.jdbc.Driver");
            con = DriverManager.getConnection(DB_URL,DB_USER,DB_PASSWORD);
            statv = con.prepareCall("{ call MSG_DETAIL(?,?,?) }");
            statv.setString(1,getidku);
            
            rs = statv.executeQuery();
            
            hasilx = statv.getString("stat");
            stv= statv.getString("st_msg");
        ...
        if(hasilx.equalsIgnoreCase("1")) {
            ...
            jo.put("com","DETAIL_MSG");
            jo.put("id_pesan",getidku);
            jo.put("status",stv);
            jo.put("data",ar);
